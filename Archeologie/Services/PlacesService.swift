 ...
 class PlacesService {
    ...
    func getThematics() {
        ...
        Observable.combineLatest(thematicsObs, query.asObservable(), locationsObs).map({ (response, query, locations) -> [Thematic] in
            ...
            let filteredLocations = locations.filter({ (place) -> Bool in
                return place.title.searchable.contains(query.searchable) || place.address.searchable.contains(query.searchable) || place.content.contains(where: { (content) -> Bool in
                    ...
                    case .text(let texts):
                        return texts.contains { $0.text.searchable.containsString(query.searchable)}
                    case .video(let videos):
                        return videos.contains { $0.text.searchable.containsString(query.searchable)}
                    case .image(let images):
                        return images.contains { $0.text.searchable.containsString(query.searchable)}
                    default:
                        return false
                    }
        ...
        Observable.combineLatest(locationsObs,selectedThematic).map { (locations, thematic) -> [Location] in
            PlacesService.service.selectedLocation.accept(nil)
